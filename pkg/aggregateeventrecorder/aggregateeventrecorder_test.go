	...
	tests := []struct {
	...
	}{
		{
			...
			exec: func(t *testing.T, ctx context.Context, subscriber types.RetrievalEventSubscriber, id types.RetrievalID) {
				...
				verifyStringNode(t, event, "storageProviderId", httpPeer.MinerPeer.ID.String())
				...
				httpPeer1Attempt, err := retrievalAttempts.LookupByString(httpCandidates[0].MinerPeer.ID.String())
				require.NoError(t, err)
				...
				httpPeer2Attempt, err := retrievalAttempts.LookupByString(httpCandidates[1].MinerPeer.ID.String())
				require.NoError(t, err)
