...
The Sheltie HTTP specification is an HTTP interface for retrieving IPLD data from IPFS and Filecoin peers. It fetches content over the GraphSync and HTTP protocols and provides the resulting data in CAR format.
...
_OPTIONAL_. `protocols=<graphsync,http>`. Defaults to all of the specified protocols returned by [IPNI](https://github.com/ipni/specs/blob/main/IPNI.md).
...
Examples:
- `protocols=graphsync` will only attempt retrievals with the graphsync protocol
